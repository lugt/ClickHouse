...
#define  ASSERT_THROW(x,b)
#define ASSERT_NO_THROW(x)
#define  EXPECT_TRUE(x)
#define  EXPECT_THROW(x,b)
...
TEST_F(TestInputStream, TestReadOneFileSameTime) {
    ...
    WriteFile(tis->fs, filename, writeSize, flag);

    for (int i = 1; i <= 50; ++i) {
        threads.push_back(
            shared_ptr<thread>(
                new thread(NothrowCheckFileContent, tis->fs, filename, readSize, 0)));
    }
...
TEST_F(TestInputStream, TestReadManyFileSameTime) {
    ...
    for (int i = 1; i <= 50; ++i) {
        ...
        ss << filename << i;
        WriteFile(tis->fs, ss.str(), writeSize, flag);
        threads.push_back(
            shared_ptr<thread>(
                new thread(NothrowCheckFileContent, tis->fs, ss.str(), readSize, 0)));
    }
