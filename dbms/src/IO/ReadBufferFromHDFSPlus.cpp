...
std::string ReadBufferFromHDFSPlus::getFileName() const
{
    return "(fd = " + toString(fd) + ")";
}


bool ReadBufferFromHDFSPlus::nextImpl()
{
    size_t bytes_read = 0;
    while (!bytes_read)
    {
        ...
        ssize_t res = 0;
        {
            ...
            // Reading
            size_t read_count = 0;
            if(isptr == nullptr)  throw Exception("BufferHDFS : Null InputStream pointer was given",ErrorCodes::ARGUMENT_OUT_OF_BOUND);
            read_count = (size_t) isptr->read(internal_buffer.begin(), internal_buffer.size());
            res = read_count;
            LOG_DEBUG(&Logger::get("StorageHDFS"), " BufferHDFS read : " << read_count); // for debug
...
off_t ReadBufferFromHDFSPlus::doSeek(off_t offset, int whence)
{
...
/*
    ...
    if (whence == SEEK_CUR)
        ...
        throw Exception("ReadBufferFromHDFSPlus::seek expects SEEK_SET or SEEK_CUR as whence", ErrorCodes::ARGUMENT_OUT_OF_BOUND);

    /// Position is unchanged.
    if (new_pos + (working_buffer.end() - pos) == pos_in_file) // which means offset = 0
        return new_pos;
...
bool ReadBufferFromHDFSPlus::poll(size_t timeout_microseconds)
{
    static_cast<void>(timeout_microseconds);
    return is_last_read_success;
